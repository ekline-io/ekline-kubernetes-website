...
- **httpGet** (HTTPGetAction)
  ...
  *HTTPGetAction describes an action based on HTTP Get requests.*
  ...
  - **httpGet.port** (IntOrString), required
    ...
    <a name="IntOrString"></a>
    *IntOrString is a type that can hold an int32 or a string.  When used in JSON or YAML marshaling and unmarshalling, it produces or consumes the inner type.  This allows you to have, for example, a JSON field that can accept a name or number.*
